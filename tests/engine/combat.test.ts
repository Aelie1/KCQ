...
describe("move validation and player actions", () => {
    ...
    it("applies a guaranteed all-player move to every party member", () => {
        ...
        expect(result.frames.map((frame) => frame.event)).toEqual([{
            ...
            targets: [
                ...
                { target: "ally", result: "none", effects: [{ type: "buffAdded", target: "ally", buff: "rallied" }] },
            ],
        }]);
    ...
    it.each(["missing", `${skunkette.id}1`])(
        ...
        (id) => {
            expect(setupAuthoredCombat().getActionView()
...
describe("move and effect resolution through GameEngine", () => {
    it("filters missed targets and normalizes successful effects before applying them", () => {
        ...
        const moveEvent = result.frames[0].event;
        ...
        expect(result.frames.at(-1)!.state.enemies.map(({ id, currHp }) => ({ id, currHp }))).toEqual([
